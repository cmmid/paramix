# adapted from Nick Golding:
# https://gist.github.com/goldingn/85d2b7363d2da2888254bb6042144150
...
participants <- polymod$participants[!is.na(year)]
taryears <- participants[, unique(year)]
tarisos <- participants[, c(countrycode(
...
get_polymod_population <- function() {
  res <- polymod$participants[!is.na(year)]
  res[, participants := .N, by = .(country, year)]
   %>%
    left_join(
      wpp_age(),
      by = c("country", "year")
