...
The SARS-COV-2 pathogen causes COVID-19, which has a notable age-specific mortality rate. In a meta-analysis, Levin et al (TODO citation) estimates the age-specific infection fatality ratio (IFR) for COVID-19 as:
...
Typically in ODE compartmental models have low-resolution stratification combining several ages, for both computational reasons and because of low-resolution date on other interactions like contact patterns.

Since we are mixing several ages into a single compartment, what is the properly blended version of parameters like IFR? The challenge here is that generally $E[f(x)] \ne f(E[x])$, so we cannot for example simply take the IFR at the mean age for the compartment. As a practical matter, the relative distribution of age varies between populations, so even the proper average of IFR needs to be weighted by that distribution.

Computing those values requires careful bookkeeping, but is not computationally expensive. This package consolidates that bookkeeping into simpler steps, so that you can focus on the bigger picture analyses.
...
# suppose some wide model age groups
model_agelimits <- c(0, 5, 20, 65, 101)
# get some real world data from wpp data sets
data('popF'); data('popM')
setDT(popF); setDT(popM)

pop_dt <- rbind(
  popF[name %like% "United Kingdom|Afghanistan", .(name, age, `2020`, source = "Females")],
  popM[name %like% "United Kingdom|Afghanistan", .(name, age, `2020`, source = "Males")]
)
