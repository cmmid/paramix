...
plot(0:100,ifr_levin(0:100),type='l',xlab='Age',ylab='Infection-fatality ratio')
```
...
For convenience, `paramix` can compute all approaches mentioned above for comparison:
...
You can see the calculations by entering `paramix::distill_summary` (no parenthesis) at an R prompt. Plotted, the results look like:
...
ggplot(distill_methods_dt) + aes(x = partition, y = value, color = method) +
  ...
  scale_color_discrete(
    ...
    labels = \(b) c(
      mean_partition = "Deaths @ Mean Group Age",
