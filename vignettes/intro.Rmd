...
In this vignette, we demonstrate applying these functions from an initial functional relationship through a final analytic process.
...
One typical measurement of public health outcomes is "years life lost" (YLL). Based on population level demography, we can estimate how many years individuals at various ages can expect to live. People that die of a modeled illness lose a corresponding number of years of life. 

For this vignette, we will work from the continuous IFR relationship, to compartmental aggregate IFR and deaths by group, back to by-year deaths and finally to estimating YLLs.

## Parametrizing a Compartmental Model

Typically in ODE compartmental models have low-resolution stratification combining several ages, for computational reasons, because of low-resolution data on other interactions like contact patterns, and to match intervention targets (e.g. children versus working age adults versus retirees).

When mixing several ages into a single compartment, what is the properly blended version of parameters like IFR? Generally, $E\left[f\left(x\right)\right] \ne f\left(E\left[x\right]\right)$, so simply using the IFR at the mean age for the compartment is not guaranteed correct. The average IFR over the age range is more correct, but that assumes uniform distribution of individuals by age, which is not generally true. The proper average of IFR needs to be weighted by the age distribution, or:

$$
\textrm{IFR}\Big\rvert_a^b = \frac{\int_a^b \textrm{IFR}(\textrm{age})\rho(\textrm{age})d\textrm{age}}{\int_a^b \rho(\textrm{age})d\textrm{age}}
$$

Let's compare the various calculations of IFR for two different population age distributions: Afghanistan and the United Kingdom. Recall, we have considered using:
...
Now let's make a plot of the IFR values for the different age groups, under different approaches to computing them. We'll use the package function `parameter_summary()`, which provides a convenient comparison of the parameter values for these approaches to summarisation:

```{r ifrplotsetup, cache=TRUE, echo=FALSE}
plot_dt <- density_dt[, {
  parameter_summary(f_param = ifr_levin, densities = .SD, model_agelimits)
}, by = name][
...
```{r params}
params <- sapply(
  c("Afghanistan", "United Kingdom"), \(nm) blend(
    ifr_levin, densities = density_dt[name == nm], model_partition = model_agelimits
  )
)
```

```{r paramstbl, echo = FALSE}
tbl <- signif(params, digits = 2)
row.names(tbl) <- paste(
  head(model_agelimits, -1), tail(model_agelimits, -1) - 1L, sep = " - "
)
knitr::kable(tbl, align = c("c", "c"), )
```
...
For demonstration purposes, let's assume 1M *infections* that happen proportionally across the model population groups[^2]. How would these translate into deaths in finer age groups, based on these different imputation assumptions? Our differently parametrised models would have converted these infections to deaths as follows:
...
model_deaths_dt <- model_density_dt[,
  .(model_from, deaths = weight*1e6*params[, name]),
  by = name
]
```
...
```{r alembicsetup, echo = FALSE, cache = TRUE, out.width = "100%", fig.width = 8, fig.height = 5, , dpi = 600}
# setup the model to outcome mapping using `alembic`s
mapping_dt <- density_dt[, alembic(
  ifr_levin, densities = .SD,
  model_partition = model_agelimits,
  new_partition = seq(min(from), max(from) + 1L)
), by = name]

distill_methods_dt <- rbind(
# approach 1: all deaths at mean age
model_deaths_dt[, .(
  name,
  age = floor((head(model_agelimits, -1) + tail(model_agelimits, -1) - 1)/2),
  deaths,
  method = "mean_age"
)],
...
density_dt[
  mapping_dt, on = .(name, from = new_from)
][model_deaths_dt, on = .(name, model_from)][, .(
  age = from, deaths = deaths * weight / sum(weight)
...
ggplot(distill_methods_dt) + aes(x = age, y = deaths, color = method) +
  facet_grid(. ~ name) +
  geom_point() +
  theme_minimal() + theme(
  ...
  scale_color_discrete(
    "Method",
    breaks = c("mean_age", "uniform_model", "alembic_weighted", "uniform_age"),
    labels = c(
      mean_age = "Deaths @ Mean Group Age",
      uniform_model = "Deaths Uniform Across Age Group",
      alembic_weighted = "paramix Approach",
      uniform_age = "Deaths Proportional to Age Distribution"
    )
  )
```
...
yll_dt <- distill_methods_dt[
  life_expectancy_dt, on = .(name, age)
][, .(YLL = sum(deaths*ex)), by = .(name, method)]
...
ggplot(yll_dt) + aes(x = name, y = YLL/1e4, fill = method) +
  ...
  ylab('YLLs per 1M Infections\n[10K person-years]') + xlab('Country')
```
